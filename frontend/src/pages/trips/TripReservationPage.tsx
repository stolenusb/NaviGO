...
export function SeatSelectionModal({ tripId, onClose, onBooked }: SeatSelectionModalProps) {
  ...
  return (
    <main
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6 backdrop-blur-sm animate-[fadeIn_180ms_ease-out]"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget) onClose();
      }}
    >
      <div className="relative w-full max-w-2xl rounded-3xl border border-gray-200 bg-white p-6 shadow-2xl animate-[scaleIn_180ms_ease-out]">
        <button
          type="button"
          onClick={onClose}
